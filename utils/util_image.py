...
def imwrite_rs(im_in, im_ref, img_name, out_dir, compress='lzw'):
    '''
    Save image using reference image's datatype and metadata with compression.
    Input:
        im_in: h x w x c, numpy tensor (input image to save)
        im_ref: path to reference geotiff image or rasterio dataset
        img_name: name for output file (without extension)
        out_dir: output directory
        compress: compression method ('lzw', 'deflate', 'jpeg', 'packbits', 'ccittfax3', 'ccittfax4', 'lzma')
    '''
    im = im_in.copy()

    # Step 1: Read reference metadata
    if isinstance(im_ref, str):
        with rasterio.open(im_ref) as src:
            metadata = src.meta.copy()
            ref_dtype = src.dtypes[0]
            transform = src.transform
    else:
        metadata = im_ref.meta.copy()
        ref_dtype = im_ref.dtypes[0]
        transform = im_ref.transform
    
    # Step 2: Smart datatype conversion
    original_dtype = im.dtype
    
    if ref_dtype != original_dtype:
        print(f"Converting from {original_dtype} to {ref_dtype}")
        
        # Get the numpy equivalent of rasterio dtype
        if hasattr(ref_dtype, 'type'):
            target_np_dtype = ref_dtype.type
        else:
            # Map rasterio dtypes to numpy dtypes
            dtype_mapping = {
                'uint8': np.uint8,
                'uint16': np.uint16,
                'int16': np.int16,
            ...
            target_np_dtype = dtype_mapping.get(str(ref_dtype), np.float32)
        
        if target_np_dtype == np.uint8:
            im = np.clip(im, 0, 255).astype(target_np_dtype)
        elif target_np_dtype == np.uint16:
            im = np.clip(im, 0, 65535).astype(target_np_dtype)
    
    # Step 3: Set up compression parameters
    compression_options = {'compress': compress}
    
    # Add quality parameter for JPEG compression
    if compress == 'jpeg':
        compression_options['jpeg_quality'] = 85  # Good quality/size balance
    
    # Step 4: Update metadata with compression
    metadata.update(
        dtype=ref_dtype,
        count=im.shape[2] if len(im.shape) == 3 else 1,
        height=im.shape[0],
        width=im.shape[1],
        **compression_options,
    ...
    output_path = os.path.join(out_dir, f"{img_name}_pred.tif")
...
def imwrite_rgb(im_in, out_path):
    def imwrite(im_in, path, chn='rgb', dtype_in='float32', qf=None):
        '''
        Save image.
        Input:
        ...
        return flag
    im_in /= 10000.0
    im_in = torch.from_numpy((im_in.transpose(2,0,1).copy())).float()
    im_in = torch.unsqueeze(im_in, 0)

    if im_in.shape[1]>=3:
        im_in_rgb = torch.flip(im_in[:,1:4,:,:],[1])
    elif im_in.shape[1] == 2:
        im_in_rgb = im_in[:,0,:,:]
        im_in_rgb = torch.unsqueeze(im_in_rgb, 0)
    else:
        im_in_rgb = im_in
    
    im_tensor = vutils.make_grid(im_in_rgb, nrow=im_in_rgb.shape[0], normalize=True, scale_each=True)
    im_np = im_tensor.cpu().permute(1,2,0).numpy()
    imwrite(im_np, out_path)
