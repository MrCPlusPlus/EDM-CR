...
class UNetModelSwin_TemporalCond(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    ...
    def __init__(
        ...
        if cond_cloudy and cloudy_size == image_size:
            self.feature_extractor = nn.Identity()
            base_chn = self.in_channels
        else:
            feature_extractor = []
            feature_chn = self.in_channels
            base_chn = 16
            for ii in range(int(math.log(cloudy_size / image_size) / math.log(2))):
        ...
        for level, mult in enumerate(channel_mult):
            for jj in range(num_res_blocks[level]):
                ...
                if self.cond_mode == 'multi' or level==0:
                    layers_cond = [
                        ResBlock(
                            ch,
                            time_embed_dim,
    ...
    def forward(self, x, timesteps, cloudy=None, s1=None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        :param cloudy: an [N x C x ...] Tensor of cloudy iamge.
        :param cloudy: an [N x C x ...] Tensor of sar iamge.
        :return: an [N x C x ...] Tensor of outputs.
        """
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels)).type(self.dtype)
        ...
        if cloudy is not None:
            assert self.cond_cloudy
            cloudy = self.feature_extractor(cloudy.type(self.dtype))
            x = th.cat([x, cloudy], dim=1)

        h = x.type(self.dtype)
        h_cond = s1.type(self.dtype)
        mode_count = 0
        for ii, module in enumerate(self.input_blocks):
            h = module(h, emb)
            hs.append(h)
            if self.cond_mode == 'multi' or mode_count==0:
                module_cond = self.input_blocks_cond[ii]
                h_cond = module_cond(h_cond, emb)
                if 'BasicLayer' in str(module_cond):
                    h = h + h_cond
                    mode_count+=1
        h = self.middle_block(h, emb)
        for module in self.output_blocks:
